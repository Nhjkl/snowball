...
]['length'];
{
  ...
  type cases = [
    Expect<Equal<num1, 4>>,
    Expect<Equal<num2, 7>>,
    Expect<Equal<num3, 1998>>,
  ];
}

// Subtract
type Subtract<X extends number, Y extends number> = buildArray<X> extends [
  ...arr1: buildArray<Y>,
  ...arr2: infer Rest,
]
  ? Rest['length']
...
> = Y extends 0 ? Result : Multiply<X, Subtract<Y, 1>, Add<X, Result> & number>;
{
...
type Divide<
  ...
  Y extends number,
  Result extends number = 0,
> = X extends 0 ? Result : Divide<Subtract<X, Y>, Y, Add<Result, 1> & number>;
...

{
  ...
  type cases = [
    Expect<Equal<num1, true>>,
    Expect<Equal<num2, false>>,
    Expect<Equal<num3, false>>,
  ];
}
